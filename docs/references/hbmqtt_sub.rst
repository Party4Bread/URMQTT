...
``urmqtt_sub`` is a command line MQTT client that subscribes to some topics and output data received from messages published.
...
``urmqtt_sub`` usage : ::
  ...
  urmqtt_sub --version
  urmqtt_sub (-h | --help)
  urmqtt_sub --url BROKER_URL -t TOPIC... [-n COUNT] [-c CONFIG_FILE] [-i CLIENT_ID] [-q | --qos QOS] [-d]
...
Note that for simplicity, ``urmqtt_sub`` uses mostly the same argument syntax as `mosquitto_sub`_.
...
-i                  The id to use for this client. If not given, defaults to ``urmqtt_sub/`` appended with the process id and the hostname of the client.
...
If ``-c`` argument is given, ``urmqtt_sub`` will read specific MQTT settings for the given configuration file. This file must be a valid `YAML`_ file which may contains the following configuration elements :
...
::
    ...
    urmqtt_sub --url mqtt://localhost -t '$SYS/#' -q 0


Subscribe to 10 messages with QoS 2 from /#:
::

    urmqtt_sub --url mqtt://localhost -t # -q 2 -n 10
...
::
    ...
    urmqtt_sub --url wss://localhost -t '$SYS/#' -q 0 --extra-headers '{"Authorization": "Bearer <token>"}'
