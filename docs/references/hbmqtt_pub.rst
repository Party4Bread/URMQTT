...
``urmqtt_pub`` is a MQTT client that publishes simple messages on a topic from the command line.
...
``urmqtt_pub`` usage : ::
  ...
  urmqtt_pub --version
  urmqtt_pub (-h | --help)
  urmqtt_pub --url BROKER_URL -t TOPIC (-f FILE | -l | -m MESSAGE | -n | -s) [-c CONFIG_FILE] [-i CLIENT_ID] [-d]
...
Note that for simplicity, ``urmqtt_pub`` uses mostly the same argument syntax as `mosquitto_pub`_.
...
-h, --help          Display ``urmqtt_pub`` usage help
...
-f                  Send the contents of a file as the message. The file is read line by line, and ``urmqtt_pub`` will publish a message for each line read.
-i                  The id to use for this client. If not given, defaults to ``urmqtt_pub/`` appended with the process id and the hostname of the client.
-l                  Send messages read from stdin. ``urmqtt_pub`` will publish a message for each line read. Blank lines won't be sent.
...
If ``-c`` argument is given, ``urmqtt_pub`` will read specific MQTT settings for the given configuration file. This file must be a valid `YAML`_ file which may contains the following configuration elements :
...
::
    ...
    urmqtt_pub --url mqtt://localhost -t sensors/temperature -m 32 -q 1

Publish timestamp and temperature information to a remote host on a non-standard port and QoS 0:
::

    urmqtt_pub --url mqtt://192.168.1.1:1885 -t sensors/temperature -m "1266193804 32"
...
::
    ...
    urmqtt_pub --url mqtt://localhost -r -t switches/kitchen_lights/status -m "on"

Send the contents of a file in two ways:
::

    urmqtt_pub --url mqtt://localhost -t my/topic -f ./data

    urmqtt_pub --url mqtt://localhost -t my/topic -s < ./data
...
::
    ...
    urmqtt_pub --url wss://localhost -t sensors/temperature -m 32 -q 1 --extra-headers '{"Authorization": "Bearer <token>"}'
