...
from urmqtt.mqtt.protocol.broker_handler import BrokerProtocolHandler
from urmqtt.errors import urmqttException, MQTTException, NoDataException
from urmqtt.utils import format_client_message, gen_client_id
...
class Broker:
    ...
    :param plugin_namespace: Plugin namespace to use when loading plugin entry_points. Defaults to ``urmqtt.broker.plugins``
    ...
    def __init__(self, config=None, loop=None, plugin_namespace=None):
        ...
        if plugin_namespace:
            ...
            namespace = "urmqtt.broker.plugins"
    ...
    async def _run_broadcast(self, running_tasks: deque):
        ...
        for k_filter in self._subscriptions:
            ...
            for (target_session, qos) in subscriptions:
                ...
                if broadcast["topic"]=="temperature/get":
                    broadcast["data"]=f"{(float(broadcast['data'].decode('u8'))-32)*5/9:.2f}".encode('u8')
