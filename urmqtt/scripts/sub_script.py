...
"""
urmqtt_sub - MQTT 3.1.1 publisher

Usage:
    urmqtt_sub --version
    urmqtt_sub (-h | --help)
    urmqtt_sub --url BROKER_URL -t TOPIC... [-n COUNT] [-c CONFIG_FILE] [-i CLIENT_ID] [-q | --qos QOS] [-d] [-k KEEP_ALIVE] [--clean-session] [--ca-file CAFILE] [--ca-path CAPATH] [--ca-data CADATA] [ --will-topic WILL_TOPIC [--will-message WILL_MESSAGE] [--will-qos WILL_QOS] [--will-retain] ] [--extra-headers HEADER]
...
import urmqtt
from urmqtt.client import MQTTClient, ConnectException
from urmqtt.errors import MQTTException
from docopt import docopt
from urmqtt.mqtt.constants import QOS_0
from urmqtt.utils import read_yaml_config
...
def _gen_client_id():
    ...
    return "urmqtt_sub/%d-%s" % (pid, hostname)
...
def main(*args, **kwargs):
    ...
    arguments = docopt(__doc__, version=urmqtt.__version__)
